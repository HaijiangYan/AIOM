...
const n_chain = 2;
const n_rest = 5;
const classes = ['happy', 'sad', 'angry', 'fearful', 'disgusted', 'surprised'];
...
const max_ss_per_class = 2; // n_trial per class will be max_ss_per_class * n_dim
////////////////////////////////////////////////////////////////////////////////////////////
...
exports.start_choices = async (req, res, next) => {
  ...
  try {
    for (let i=1; i<=n_chain; i++) {
      ...
      await pool.query(
        `INSERT INTO ${table_name} (sample, current_dim) 
        VALUES ($1, $2)`,
        [JSON.stringify(starting_point), 0]
      );
    }
...
exports.get_choices = async (req, res, next) => {
  ...
  try {
    var current_state, current_dim;

    const result_ = await pool.query(`
      SELECT sample, current_dim FROM ${table_name} ORDER BY id DESC LIMIT 1
      `);

    // console.log(result_.rows[0].sample, typeof result_.rows[0].sample);
...
exports.register_choices = async (req, res, next) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO ${table_name} (sample, current_dim) 
      VALUES ($1, $2)`,
      [JSON.stringify(selected), current_dim+1]
    );
    const ss = Math.floor(n_trial / n_dim);
    if (ss < max_ss_per_class) {
      res.status(200).json({"finish": 0, "progress": n_trial/(max_ss_per_class*n_dim)});
    } else {
